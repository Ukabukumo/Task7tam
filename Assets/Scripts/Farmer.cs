...
{
    [SerializeField] private Sprite[]   _farmerFrames;
    [SerializeField] private Sprite[]   _farmerDirtFrames;
    [SerializeField] private Sprite[]   _farmerAngryFrames;
    ...
    private readonly Vector3 _itemOffset = new Vector3(0f, -0.5f, 0f);
    ...
    private void Movement()
    {
        ...
        if (_targetPos > 0)
        {
            ...
            // So that the Farmer hides behind other objects and vice versa
    ...
    private void RandTurn()
    {
        ...
        switch (direction)
        {
            ...
            case 0:
                _rX = -1; _rY = 0;
                _targetPos = new Vector2(2f, 0f).magnitude;
                break;
            ...
            case 2:
                _rX = 0; _rY = -1;
                _targetPos = new Vector2((_moveSpeed / 450f) * 2, 1.8f).magnitude;
                break;
    ...
    {
        Sprite[] frames = _farmerFrames;

        if (_isAngry)
        {
            frames = _farmerAngryFrames;
        }
    ...
    {
        // Ignore another enemies
        if ((collision.gameObject.tag == "Farmer") ||
            (collision.gameObject.tag == "Dog"))
        {
    ...
    IEnumerator DeathTimer()
    {
        ...
        {
            case (1, 0):
                GetComponent<SpriteRenderer>().sprite = _farmerDirtFrames[0];
                break;

            case (-1, 0):
                GetComponent<SpriteRenderer>().sprite = _farmerDirtFrames[1];
                break;

            case (0, -1):
                GetComponent<SpriteRenderer>().sprite = _farmerDirtFrames[2];
                break;

            case (0, 1):
                GetComponent<SpriteRenderer>().sprite = _farmerDirtFrames[3];
                break;
        }
        ...
        if (chance < 10)
        {
            Instantiate(_fireBonusPref, transform.position + _itemOffset, Quaternion.identity, _lm._items);
        }

        else
        {
            Instantiate(_coinPref, transform.position + _itemOffset, Quaternion.identity, _lm._items);
        }
